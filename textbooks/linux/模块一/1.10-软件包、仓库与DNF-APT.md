...
3. 说明切换国内镜像前后的备份、验证和回退流程，并能够按教师提供的课程基线处理仓库故障。
...
下面是全系统升级命令的写法，本课程只要求识别，不在普通课堂实验中统一执行：
...
`apt update`只更新本地软件索引，不等于升级已安装软件。升级软件使用下面的命令。本课程只要求识别，不在普通课堂实验中统一执行：
...
## 1.10.4 配置国内镜像源并验证回退（参考操作）

本节涉及系统软件源修改，用于解释“备份—修改—刷新—验证—回退”的完整方法，不是实验5必做步骤。课堂短练习和实验5只做查询、备份与事务验证；只有课程初始源失效，并且教师已经确认镜像地址与机房网络可用时，才按本节处理，不能把不同版本的仓库配置混用。
...
实验5在`rocky-server`上完成仓库基线、配置备份、元数据刷新、`tree`和`jq`安装、包文件查询、受控卸载与DNF历史留证，不重复切换软件源。所有系统修改和`~/m1-project/evidence`成果以实验5步骤为准；Rocky与Ubuntu换源均以实验1建立的课程镜像基线为准。
