...
2. 在VMware中安装Rocky Linux 9和Ubuntu Server 22.04，完成统一NAT网段与固定地址配置。
...
| Ubuntu对照虚拟机 | 2 vCPU、2-4GB内存、40GB磁盘、NAT网络 |
| 主线账号 | 普通用户`student`，具备sudo权限 |
| 终端工具 | VMware控制台；后续可使用Windows Terminal、Xshell或MobaXterm |

所有示例默认在Rocky Linux 9中执行。标有“Ubuntu对照”的命令需要在Ubuntu Server 22.04中执行。

## 学习路线与成果

本模块对应12个学习单元，每个单元建议投入2学时。每章均包含可直接执行的实践任务或验收要求，本文件本身即可完成模块学习；仓库中的独立实验文件仅作为追加练习，不是完成本教材的前置条件。

| 单元 | 教材章节 | 本章实践 | 应形成的成果 |
|---|---|---|---|
| U01 | 第1章 | 系统信息调查 | 系统与发行版识别表 |
| U02 | 第2章 | 安装与初始化 | 可回退的Rocky主线虚拟机与Ubuntu对照虚拟机 |
| U03 | 第3章 | 命令行与发行版对照 | 双发行版命令对照记录 |
| U04 | 第4章 | 目录和文件管理 | 规范的运维项目目录树 |
| U05 | 第5章 | 查看、筛选、查找与链接 | 文件查找、链接与重定向结果 |
| U06 | 第6章 | vim与归档恢复 | 可验证恢复的归档文件 |
| U07 | 第7章 | 项目团队账号任务 | 用户、组与密码策略记录 |
| U08 | 第8章 | 共享目录权限 | 共享目录权限验证记录 |
| U09 | 第9章 | 最小sudo授权 | 最小sudo授权文件与审计记录 |
| U10 | 第10章 | 包管理与镜像源 | 包管理对照及镜像源回退记录 |
| U11 | 第11章 | 自定义systemd服务 | 自定义服务、故障与日志证据 |
| U12 | 第12章 | 状态巡检与综合交付 | 系统巡检脚本与基础服务器交付报告 |
...
├── 软件源与基础软件记录
├── systemd服务操作记录
├── ~/m1-project/scripts/m1-health-check.sh  系统巡检脚本
└── Linux基础服务器交付报告
```

每章先学习必要原理和命令，再完成项目中的一个交付阶段。前文创建的虚拟机、目录、文件、账号和配置会在后文继续使用；除非教材明确要求，不要提前删除。
...
NAT模式把虚拟机放入VMnet8私有网络。虚拟机之间可以通信，也可以经宿主机访问外部网络；外部局域网默认不能直接向虚拟机发起连接。课程统一网段和固定地址后，每台机器的实验结果更容易复现。
...
本教材后续统一使用下面的地址，不再让每台虚拟机随机获取不同地址：

| 对象 | 固定值 |
|---|---|
| VMnet8网络 | `192.168.200.0/24` |
| 子网掩码 | `255.255.255.0` |
| Windows上的VMware虚拟网卡 | 通常为`192.168.200.1` |
| NAT网关 | `192.168.200.2` |
| Rocky Linux | `192.168.200.10` |
| Ubuntu Server | `192.168.200.20` |
| DHCP地址池 | 建议`192.168.200.128-192.168.200.254` |
| DNS | `223.5.5.5`、`119.29.29.29` |

静态地址`.10`和`.20`位于DHCP地址池之外，可以避免DHCP把同一地址分给其他虚拟机。
...
Windows宿主机物理网卡
      │ NAT地址转换
VMware VMnet8：192.168.200.0/24
├── 宿主机虚拟网卡：192.168.200.1
├── NAT网关：192.168.200.2
├── Rocky Linux：192.168.200.10
└── Ubuntu Server：192.168.200.20
```

先关闭正在使用VMnet8的虚拟机，再在VMware Workstation中打开“编辑 → 虚拟网络编辑器”，必要时选择“更改设置”取得管理员权限，然后：

1. 选择`VMnet8`。
2. 网络类型选择NAT。
3. 子网IP设置为`192.168.200.0`。
4. 子网掩码设置为`255.255.255.0`。
5. 在“NAT设置”中确认网关为`192.168.200.2`。
6. 在“DHCP设置”中把地址池调整到`.128-.254`，或保留DHCP但确保不包含`.10`和`.20`。
7. 应用配置。

> **[截图占位 M1-01：桥接、NAT、仅主机模式关系图]**

> **[截图占位 M1-02：VMware虚拟网络编辑器中的VMnet8子网、NAT网关和DHCP范围]**

Windows执行`ipconfig`，应能看到VMware Network Adapter VMnet8及`192.168.200.1`。如果显示其他网段，先修正VMnet8，不要继续安装后再猜测网关。
...
5. 在IPv4设置中选择Manual，填写地址`192.168.200.10/24`、网关`192.168.200.2`、DNS`223.5.5.5,119.29.29.29`。
6. 保存后确认安装界面显示网卡已连接且地址为`192.168.200.10`。
7. 设置root密码。
8. 创建普通用户`student`，并授予管理员权限。
9. 开始安装，完成后重启并断开ISO。

> **[截图占位 M1-04：Rocky安装器中192.168.200.10静态地址、网关和DNS]**
...
- Rocky地址为`192.168.200.10/24`。
- 默认路由为`default via 192.168.200.2`。
- DNS包含课程指定地址。

安装器已经创建NetworkManager连接，不需要再额外创建`static-net`或`auto-net`。查看实际网卡和连接名：
...
如果地址填错，应在VMware控制台修改现有连接，而不是叠加多个连接：

```bash
IFACE=$(ip route show default | awk 'NR==1 {print $5}')
CON_NAME=$(nmcli -g GENERAL.CONNECTION device show "$IFACE")
printf 'interface=%s connection=%s\n' "$IFACE" "$CON_NAME"
sudo nmtui edit "$CON_NAME"
```

在nmtui中修正IPv4地址、网关和DNS，保存后执行`sudo nmcli connection up "$CON_NAME"`。远程连接断开时必须使用VMware控制台处理。
...
快照完成后重新启动Rocky，保持它在线，再安装Ubuntu并进行双机验证。

## 2.6 安装Ubuntu Server 22.04对照环境

Ubuntu Server使用文本式Subiquity安装程序。核心步骤：

1. 使用`ubuntu-22.04-live-server-amd64.iso`启动。
2. 选择语言和键盘。
3. 网络选择手动IPv4：Subnet填写`192.168.200.0/24`，Address填写`192.168.200.20`，Gateway填写`192.168.200.2`，Name servers填写`223.5.5.5,119.29.29.29`。
4. 使用整个虚拟磁盘。
5. 软件源镜像地址使用课程统一的清华源或阿里源，例如`https://mirrors.aliyun.com/ubuntu`。
6. 创建`student`账号和主机名`ubuntu-vm`。
7. 勾选安装OpenSSH Server。
8. 不选择额外Snap，完成安装并重启。

> **[截图占位 M1-06：Ubuntu Server的192.168.200.20静态地址和镜像源配置]**
...
验证顺序是“本机地址→NAT网关→另一台虚拟机”，不要一开始只测试互联网地址。
...
- [ ] `student`可以使用sudo。
- [ ] VMnet8固定为`192.168.200.0/24`，NAT网关为`.2`。
- [ ] Rocky固定为`.10`，Ubuntu固定为`.20`，地址不重复。
- [ ] Rocky、Ubuntu和NAT网关可以按规划互通。
- [ ] `open-vm-tools`处于active状态。
- [ ] 已创建干净快照。
- [ ] Ubuntu Server 22.04对照虚拟机已安装并进入纯命令行。
...
1. 主机名保持为`rocky-vm`，固定地址保持为`192.168.200.10/24`。
