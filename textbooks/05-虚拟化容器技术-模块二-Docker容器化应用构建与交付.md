...
本册按照第6—11章顺序学习。各章按“交付问题—对象与机制—配置和命令入口—数据或网络路径—风险—排障—迁移”组织；章节练习验证单项能力，实验6—11再将这些能力组合成可迁移、可恢复的容器化交付项目。

教材练习使用`~/course-practice/vc/`下的独立目录和明确的课程镜像。实验产生的镜像、网络、卷、源码、Dockerfile和Compose目录具有连续关系；清理前必须先区分练习对象和项目对象。课程Registry不可用时，使用带清单和SHA256的离线包恢复，不临时改用公网`latest`镜像。
