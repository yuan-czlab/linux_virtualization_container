...
> 教师用详细资源版 | 新进度表核心实施为 U69-U72 · 4次大课 · 8课时
> 学生使用任务书（project02-综合项目-学生版.md）和可运行基础工程（project02-starter/）
> 本文保留7个资源阶段供教师备课与拆分，并非7次新增课堂。阶段1-3由Linux课程成果复用，阶段4及Compose基础由U52、U65-U67成果复用；U69-U72只实施设计恢复、容器化部署、故障验收和答辩归档。
...
核心部署方式：在web-server使用Docker Compose编排4个服务（frontend / backend / db / redis）；client负责验收，db-server保存备份和归档。

### 当前实施基线

- 学生任务书：`project02-综合项目-学生版.md`。
- 可运行工程：`project02-starter/`。
- 核心模式是单Docker主机Compose，不宣称Compose可以跨主机编排。
- 双主机拆分为两个Compose项目只作为选做挑战。
- 本文中的大段命令用于教师讲解原理和准备故障，不要求学生在U69-U72重新逐行录入。
- 如果本文代码与starter不一致，以starter和学生任务书为准。

---

## 资源阶段1：选题与Linux基础环境（复用Linux课程成果）

### 环节1：项目发布（20分钟）

**教师讲解**：
1. 展示最终效果 —— 浏览器访问 `http://VM_IP` 看到官网+留言板
2. 说明学生任务书中的10项核心任务和10项交付物
3. 分组（2-3人/组），每组选题（4选1，或选"TechCorp官网+留言板"）
4. 评分标准说明（架构环境15% + 容器服务25% + 健康安全15% + 验收排障20% + 恢复归档15% + 文档答辩10%）
...
## 资源阶段2：网络与SSH + Nginx部署（复用Linux课程成果）
...
## 资源阶段3：数据库 + Redis + Shell脚本（复用Linux课程成果）
...
## 资源阶段5：Docker命令级原理演示（U70按starter核心实施）
...
## 资源阶段6：Docker Compose 编排 + 排障（U71使用starter实施）
